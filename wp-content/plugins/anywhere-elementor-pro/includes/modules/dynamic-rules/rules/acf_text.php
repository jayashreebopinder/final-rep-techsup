...
class Acf_Text extends RuleBase {
	...
	public function get_multiple_name_control(){
		$acf_fields = Aepro::$_helper->ae_get_acf_fields(['text','url','text_area','wysiwyg','email','number','password']);
		$repeater_fields = Aepro::$_helper->get_acf_repeater_field();
		$group_field = Aepro::$_helper->get_group_fields();
		$supported_field = ['text','url','text_area','wysiwyg','email','number','password'];
		$controls = [
			[
				'label'       => 'Parent',
				'type'      => Controls_Manager::SELECT,
